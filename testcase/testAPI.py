...
class Demo_API(unittest.TestCase):
    ...
    def test_api(self,data):
        ...
        if readData_code != self.result['code' or 'status'] or readData_msg != self.result['msg'or 'error']:
            NOT_data = "FAIL"
            print("用例测试结果:  {0}---->{1}".format(data['ID'], NOT_data))
            WriteExcel(setting.TARGET_FILE).write_data(rowNum + 1,NOT_data)
        self.assertEqual(self.result['code' or 'status'], readData_code, "返回实际结果是->:%s" % self.result['code' or 'status'])
        self.assertEqual(self.result['msg'or 'error'], readData_msg, "返回实际结果是->:%s" % self.result['msg'or 'error'])
